<?php
/**
 * Customer refunded order email - Aromatic Scents Lab Custom Style
 *
...
<p class="email-text" style="font-size: 14px; line-height: 1.7; color: #c0392b; margin: 0 0 15px 0;">
	...
	if ( isset( $partial_refund ) && $partial_refund ) {
		printf(
			...
			esc_html__( 'Your order at %s has been partially refunded. There are more details below for your reference.', 'woocommerce' ),
			esc_html( get_bloginfo( 'name', 'display' ) )
		);
	} else {
		printf(
			/* translators: %s: Site title */
			esc_html__( 'Your order at %s has been refunded. There are more details below for your reference.', 'woocommerce' ),
			esc_html( get_bloginfo( 'name', 'display' ) )
		);
	}
	?>
